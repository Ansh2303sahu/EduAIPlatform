"""Student Phase 12 graph — full execution pipeline.
...
Why final_guardrail runs before persistence:
  persist_ai_report inserts a row unconditionally. Running it before the guardrail
  can write FAILED or BLOCKED rows with no final_status set and an incomplete trace
  (state.terminal_trace is None until finalize_trace() is called by final_guardrail).
  Moving persistence after the guardrail means:
  - Only guardrail-approved outputs (COMPLETED or PARTIAL) are written.
  - The persisted model_versions.graph_trace includes the finalized terminal_trace.
  - BLOCKED/FAILED executions do not pollute ai_reports with empty rows.

Execution nodes are wrapped in ``_safe_exec`` which:
  1. Checks ``terminal_decision(state).should_stop`` before calling the node —
     skips gracefully when the graph has already exceeded a loop or step limit.
  2. Catches any unhandled exception, records it via ``record_error``, and
     returns state so the graph degrades rather than crashing.

The module-reference import pattern (``from ..nodes import ingestion as
_ingestion_mod``) is used for all execution nodes so that test patches applied
to the attribute on the source module (e.g.
``patch("app.langgraph.nodes.ingestion.run", ...)``) are resolved at call time
...
# Synchronous, deterministic, no external I/O.
# All return a string mapping to a registered edge target.
# ---------------------------------------------------------------------------
...
def _route_after_ml_decision(state: Phase12GraphState) -> str:
    ...
    ``ml_decision_node`` always sets ``state.ml_required`` explicitly, so
    this route is a direct flag read with no external I/O.
    """
...
def _route_after_rag_decision(state: Phase12GraphState) -> str:
    """Exit on cross-role retrieval block; otherwise branch to retrieval or skip.

    ``rag_decision_node`` is the last control node that can set BLOCKED status.
    """
    if (
...
def _mcp_allowed(state: Phase12GraphState) -> bool:
    """Return True when MCP tool use is permitted by settings and policy."""
    return (
        phase12_settings.allow_mcp_tools
        and phase12_settings.allow_student_mcp_tools
...
def _route_after_final_guardrail(state: Phase12GraphState) -> str:
    ...
    BLOCKED and FAILED skip persistence entirely — no usable report exists and
    writing an empty row would create audit noise with an incomplete trace.
    COMPLETED and PARTIAL both have a guardrail-approved report worth persisting.
    At this point state.terminal_trace is fully populated (finalize_trace() was
    called by final_guardrail_node), so the persisted model_versions includes the
    complete trace.
    """
    fs = state.final_status or state.status
    if fs in (GraphExecutionStatus.BLOCKED, GraphExecutionStatus.FAILED):
        return "end"
    # COMPLETED or PARTIAL — persist the guardrail-approved output
    return "persistence"
...
def _route_after_persistence(state: Phase12GraphState) -> str:
    ...
    All three branches resolve to END. Distinct labels allow observers to identify
    the terminal outcome from the graph trace without inspecting state.
    PARTIAL + safe_mode → safe_mode_completed; PARTIAL without safe_mode → failed
    (the guardrail would have emitted COMPLETED for a clean, non-safe-mode run).
    """
    fs = state.final_status or state.status
...
def _build_student_graph() -> StateGraph:
    """Assemble the full student Phase 12 StateGraph without compiling.

    Keeping build and compile separate allows tests to inspect graph topology
    (nodes, edges) before triggering compile-time validation overhead.
    """
    ...
    # Persistence runs only after the guardrail has approved the output and set
    # state.terminal_trace; BLOCKED/FAILED routes skip persistence entirely.
    g.add_conditional_edges(
        "final_guardrail",
...
def get_student_compiled_graph():
    """Compile and return the full student Phase 12 graph.

    The compiled graph accepts a Phase12GraphState and after invocation returns
    a dict reconstructable via ``Phase12GraphState.model_validate()``.
    Callers should cache this result — compilation validates the full topology.
    """
    return _build_student_graph().compile()
...
STUDENT_GRAPH_SPEC = Phase12GraphSpec(
    name="phase12_student_graph",
    role="student",
    description=(
        "Full execution LangGraph for the student pipeline. "
        ...
        "output validation, optional fallback and MCP extension, and persistence."
    ),
...
def get_student_graph_spec() -> Phase12GraphSpec:
    """Return the deterministic student graph specification."""
    return STUDENT_GRAPH_SPEC
