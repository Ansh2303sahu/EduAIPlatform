"""Phase 15/16 student generative graph.

This graph stays additive to the current Phase 10 and earlier Phase 12 work.
It assumes ingestion, ML, and RAG state have already been prepared by the
service layer and focuses on bounded control + structured generation.
"""
...
@lru_cache(maxsize=1)
def get_student_generative_compiled_graph():
    return build_student_generative_graph().compile()
