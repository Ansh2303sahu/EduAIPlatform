"""
LangChain professor generation router.

Primary public path:
- POST /api/langchain/professor/generate

Compatibility alias:
- POST /api/phase10/professor/generate
"""
...
from app.services.report_generation_support import (
    ...
    call_ai_professor_multimodal as _call_ai_professor_multimodal,
...
logger = logging.getLogger("langchain.router.professor")

_pipeline = ProfessorPipeline()


@router.post("/professor/generate", response_model=LangChainProfessorResponse)
async def generate_professor(
    body: Phase10GenerateIn,
    user: CurrentUser = Depends(require_roles("professor", "admin")),
):
    return await _generate_professor(body, user)


@legacy_router.post(
    "/professor/generate",
    response_model=LangChainProfessorResponse,
    include_in_schema=False,
)
async def generate_professor_legacy(
    body: Phase10GenerateIn,
    user: CurrentUser = Depends(require_roles("professor", "admin")),
):
    return await _generate_professor(body, user)
...
async def _generate_professor(
    ...
    analysis_type = (
        "professor_project_review"
        if submission_kind == "project"
        else "professor_academic_review"
    )
    ...
    prompt_hash = _sha256_json(
        {
            "role": "professor",
            "template": "langchain_professor_v1",
    ...
    if not body.force:
        existing = await _get_rows(
            "ai_reports"
            f"?file_id=eq.{body.file_id}&role=eq.professor"
            f"&input_hash=eq.{input_hash}&prompt_hash=eq.{prompt_hash}"
            "&select=*&order=created_at.desc&limit=1"
        )
        if existing:
            normalized = _normalize_report_row("professor", existing[0]) or {}
            response = _build_cached_professor_response(
                request_id=request_id,
                stored_row=normalized,
            )
            return LangChainProfessorResponse.model_validate(response)
    ...
    logger.info(
        "langchain professor generate complete file_id=%s request_id=%s model=%s total_ms=%d",
        body.file_id,
        request_id,
    ...
    response = {
        ...
        "meta": _public_meta_from_result("professor", result),
        "stored": _public_stored_summary(stored),
    }
    return LangChainProfessorResponse.model_validate(response)


def _build_cached_professor_response(
    *,
    request_id: str,
