...
def _np_f32(x: Any, shape: Optional[Tuple[int, ...]] = None) -> np.ndarray:
...
def _np_i64(x: Any, shape: Optional[Tuple[int, ...]] = None) -> np.ndarray:
    arr = x if isinstance(x, np.ndarray) else np.asarray(x)
...
def onnx_predict_multimodal(
    ...
    *,
    ...
    Inputs:
      ...
      - mask: [B,4] bool or int (True/1 means modality present)
    ...
    # mask must be int64 for ONNX inputs in your model
    if mask is None:
        # default: text only
    ...
    if output_index >= len(outputs):
        raise IndexError(f"Requested output_index={output_index}, but model returned {len(outputs)} outputs")
...
def onnx_predict_multimodal_multitask(
    ...
    *,
    ...
    """
    Multi-head predictor for multitask ONNX export where the model returns:
      outputs = [logits_head0, logits_head1, logits_head2, ...]
